...
public class Progressbar extends Tooltip {
	...
	public void drawBackground(PoseStack stack, int mx, int my, float t) {
		...
		case H_FILL_R:
			if (n >= w) break;
			if (rev) parent.drawRect(stack, x + w - n, y, tx, ty, n, h);
			else parent.drawRect(stack, x, y, tx + w - n, ty, n, h);
			return;
		case H_FILL: 
			if (n >= w) break;
			if (rev) parent.drawRect(stack, x + w - n, y, tx + w - n, ty, n, h);
			else parent.drawRect(stack, x, y, tx, ty, n, h);
			return;
		case V_FILL_R:
			if (n >= h) break;
			if (rev) parent.drawRect(stack, x, y, tx, ty + h - n, w, n);
			else parent.drawRect(stack, x, y + h - n, tx, ty, w, n);
			return;
		case V_FILL:
			if (n >= h) break;
			if (rev) parent.drawRect(stack, x, y, tx, ty, w, n);
			else parent.drawRect(stack, x, y + h - n, tx, ty + h - n, w, n);
			return;
		case H_SLIDE: parent.drawRect(stack, x, y, tx + w - n, ty, w, h); return;
		case V_SLIDE: parent.drawRect(stack, x, y, tx, ty + n, w, h); return;
		case PIXELS:
			int m = n / h; n %= h;
			if (m >= w) break;
			if (rev) {
				parent.drawRect(stack, x + w - m, y, tx + w - m, ty, m, h);
				int dx1 = w - m - 1, dy1 = h - n;
				parent.drawRect(stack, x + dx1, y + dy1, tx + dx1, ty + dy1, 1, n);
			} else {
				parent.drawRect(stack, x, y, tx, ty, m, h);
				parent.drawRect(stack, x + m, y, tx + m, ty, 1, n);
			} return;
		}
		parent.drawRect(stack, x, y, tx, ty, w, h);
	}
